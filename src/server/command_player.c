...
void		command_ppo(t_server *server, t_client *client, char *arg)
{
  ...
  if (!(p = get_player_by_id(server->players, player_id)))
    {
      send_socket(client->fd, "sbp\n");
      return;
    }
  if (!(b = malloc(sizeof(char) * 100)))
    return;
  snprintf(b, 100, "ppo %d %d %d %d\n", p->id, p->x, p->y, (int)p->direction);
  send_socket(client->fd, b);
  free(b);
}
